...
export const getWorkers = asyncErrorHandler(async (req, res,next) => {
  ...
  // const cachedWorkers = await redis.get('workers');
  // if (cachedWorkers) {
  ...
  // await redis.set('workers', JSON.stringify(workers), 'EX', 3600);
  res
  .status(200)
...
export const getParticularWorker = asyncErrorHandler(async (req, res,next) => {
    const {id} = req.params;
    if(!id){
      return next(new ErrorHandler("Please provide a valid id", 400))
    }
   
  // const cachedWorker = await redis.get(`worker:${id}`);
  // if (cachedWorker) {
  //   // If data is found in cache, parse and return it
  //   return res.status(200).json({
  //     message: "Worker retrieved from cache",
  //     success: true,
  ...
  // }
    ...
    // await redis.set(`worker:${id}`, JSON.stringify(worker), 'EX', 3600);
    res
    .status(200)
